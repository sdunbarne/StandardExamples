...
\begin{enumerate}
    ...
    \item
        ...
        \).
    \item
        In a \defn{random walk on a circle} if the particle reaches
        site \( 0 \), then at the next step it is at \( n \) or \( 1 \)
        with probability \( q \) or \( p \) respectively.  If the
        particle reaches site \( n \), then at the next step it is at \(
        n-1 \) or \( 1 \) with probability \( q \) or \( p \)
        respectively.
    \item
    ...
    \item
        ...
        from one of two urns with all selections equally likely, and moved from
        the urn it is in to the other urn.  The state from \( 0 \) to
        \( N \) at each time is
        the number of balls in a fixed urn.
\end{enumerate}
...
\[
    ...
    \end{pmatrix}
    .
\] If \( p_i = 0 \) for \( i > N \), then the state space of the Markov
chain is finite, with corresponding \( N \times N \) finite transition
probability matrix.

In this example, make the sensible assumption that \( 0 < p_i < 1 \) for the
set of states, either finite or infinite, so that only states with a
probability of appearing are considered.  The stationary distribution is
\( p_i > 0 \).  Note that \( P^n = P \).  With the assumption,
...
\begin{align*}
  ...
  & = \Prob{\xi_1 + \xi_2 + \xi_3 +
    \cdots + \xi_n + \xi_{n+1} = j \given \xi_1 + \xi_2 + \xi_3 +
    \cdots + \xi_n = i} \\
  & = \Prob{\xi_{n+1} = j-i}
\end{align*}    
...
of integers \( \set{0, 1, 2, 3 \dots, n} \).%
...
1 \).  The transitions at the boundaries \( 0 \) and \( n \) get special
...
\[
    ...
    \begin{pmatrix}
        ...
        \vdots & \vdots \\
        0 & 0 & 0 & 0 & 0 & \cdots & 0 & q_n & r_n \\
    \end{pmatrix}
\]

If \( r_0 = r_n = 1 \) the particle reaches sites \( 0 \) or \( n \),
...
This Markov chain is not ergodic because of the absorbing boundaries.
In the case that \( r_i = 0 \), the period of Markov chain is \( 2 \).
If \( r_i = 0 \) the chain is not regular.  All states are accessible
from states \( i \) for \( 0 < i < n \), but because of the absorbing
boundaries, not all states communicate.  The transient states are \( 1,
2, \dots, n-1 \) and the absorbing states are \( 0 \) and \( n \).
...
This Markov chain has a stationary distribution.  This Markov chain is
...
integers \( \set{0, 1, 2, 3 \dots, n} \).  A particle at%
\index{random walk! on a circle}
\index{Markov chain ! random walk on a circle}
site \( i \) with \( 1 < i < n-1 \) moves to adjacent site \( i+1 \)
with probability \( p \) and to adjacent site \( i-1 \) with probability
\( q \) with \( p+q = 1 \).  If the particle reaches sites \( 0 \), then
at the next step it is at \( n \) or \( 1 \) with probability \( q \) or
\( p \) respectively.  If the particle reaches sites \( n \), then at
the next step it is at \( n-1 \) or \( 1 \) with probability \( q \) or \(
...
\[
    ...
    \end{pmatrix}
\] This is also a random walk with state space \( \Integers_{n+1} \).

  This Markov chain has a stationary distribution
  \[ (1/(n+1), 1/(n+1),\dots, 1/(n+1)).
  \]  This Markov chain is ergodic.  The chain is
periodic with period \( 2 \) so it is not regular.  All states are
...
infinite set of nodes, also called vertices, \( \set{v_1, v_2, \dots, v_n}
\), and an associated collection of edges \( \set{v_{ij}, 1 \le i,j \le
n} \).  There need not be an edge between any two vertices, if there is
a connection between vertices \( v_i \) and \( v_j \), then \( v_j \) is
...
\binom{N}{i}/2^N \). (See the exercises.)  This is the binomial distribution on \( N \),
...
chain is periodic with period \( 2 \) so it is not regular.  All states
are recurrent with no transient states.  This Markov chain has no
absorbing states.

\subsubsection*{An alternative Ehrenfest Urn Model}
% Karlin and Taylor, Chapter 2, Problem 2 (a), page 74

Two urns labeled \( A \) and \( B \), contain a total of \( N \) balls.
...
\[
    ...
    \begin{pmatrix}
        q & p & 0 & 0 & \cdots & 0 & 0 \\
        \frac{1}{N} q & \frac{1}{N}p + \left( 1-\frac{1}{N} \right) q &\left
        ( 1-\frac{1}{N} \right) p & 0 & \cdots & 0 & 0 \\
        0 & \frac{2}{N} q &\frac{2}{N} p + \left( 1-\frac{2}{N} \right)
        q &\left( 1-\frac{2}{N} \right) p & \ dots & 0 & 0 \\
        0 & 0 & \frac{3}{N} q & \frac{3}{N} p + \left( 1-\frac{3}{N}
        \right) q & \cdots & 0 & 0 \\
        \vdots & \vdots & \vdots & \vdots & \ddots & \vdots & \vdots \\
        0 & 0 & 0 & 0 & \cdots & q & p \\
    \end{pmatrix}
...
p \), \( i \), and \( N \) in a more complicated relationship than in
...
\subsubsection*{Another Generalized Ehrenfest Urn Model}
% Karlin and Taylor, Chapter 2, Problem 2 (b), page 74

Two urns labeled \( A \) and \( B \) contain a total of \( N \) balls.
At time \( t \) there \( k \) balls in urn \( A \).  At time \( t + 1 \)
an urn is selected, urn \( A \) with probability \( k/N \) and urn \( B \)
with probability \( (N-k)/N \).  Then independently a ball is selected
from urn \( A \) with probability \( p \) or from urn \( B \) with
probability \( q = 1-p \) and placed in the previously selected urn. The
state at each time is the number of balls in the urn \( A \), from \( 0 \)
to \( N \).  Then the transition probability matrix is
\[
    P =
    \begin{pmatrix}
        1 & 0 & 0 & 0 & \cdots & 0 & 0 \\
        \frac{N-1}{N} p & \frac{N-1}{N} q + \left( \frac{1}{N} \right) p
        & \left( \frac{1}{N} \right) q & 0 & \cdots & 0 & 0 \\
        0 & \frac{N-2}{N} p & \left( 1-\frac{2}{N}\right) q + \frac{2}{N} p
         & \left( \frac{2}{N} \right) p & \cdots & 0 & 0 \\
        0 & 0 & \frac{N-3}{N} p & \left( \frac{N-3}{N}\right) q + \frac{3}{N} p 
        & \cdots & 0 & 0 \\
        \vdots & \vdots & \vdots & \vdots & \ddots & \vdots & \vdots \\
        0 & 0 & 0 & 0 & \cdots & 0 & 1 \\
    \end{pmatrix}
...
This Markov chain is not ergodic.  States \( 1 \) through \( N-1 \)
communicate while states \( 0 \) and \( N \) are absorbing.  States \( 1
\) to \( N-1 \) are transient states.  The chain is periodic with period
\( 1 \).
...
an urn is selected, urn \( A \) with probability \( k/N \) and urn \( B \)
with probability \( (N-k)/N \).  Then independently a ball is selected
from urn \( A \) with probability \( k/N \) or from urn \( B \) with
probability \( (N-k)/N \) and placed in the previously selected urn.
The state at each time is the number of balls in the urn \( A \), from \(
0 \) to \( N \).  Then the transition probability matrix is
\[
    P =
    \begin{pmatrix}
        1 & 0 & 0 & 0 & \cdots & 0 & 0 \\
        \frac{1 \cdot(N-1)}{N^2} & \frac{1^2}{N^2} +\frac{(N-1)^2}{N^2}
        & \frac{(N-1) \cdot 2 }{N} & 0 & \cdots & 0 & 0 \\
        0 & \frac{2 \cdot (N-2)}{N} & \frac{2^2}{N^{2}} + \frac{(N-2)^2}
        {N^2} & \frac{(N-2) \cdot 2}{N^{2}} & \cdots & 0 & 0 \\
        0 & 0 & \frac{3 \cdot (N-3)}{N^2} & \frac{3^2}{N^{2}} + \frac{(N-3)^2}
        {N^2} & \cdots & 0 & 0 \\
        \vdots & \vdots & \vdots & \vdots & \ddots & \vdots & \vdots \\
        0 & 0 & 0 & 0 & \cdots & 0 & 1 \\
    \end{pmatrix}
...
pages 45--80,
\cite{karlin75}; with more information and
ideas from \booktitle{Problems in Probability Theory, Mathematical
Statistics and Theory of Random Functions} by A. A. Svesnikov, Chapter
VIII, pages 231--274,
\cite{svesnikov68}
...
\begin{codebox}
  \Procname{Standard Markov Chain Examples}
  \zi Comment Post: Simulation and information about standard Markov
  chains
  \li Load Markov chain library
  \li Set state names
  \li Set transition probability matrix
  \li Set start state
  \li Set up Markov chain with the library
  \li Set an example length
  \li \Return A sample path of example length
  \li \Return A second sample path of example length
  \li \If the Markov chain has no transients, i.e.\  all recurrent 
  \li     Set a long path length, and a transient time
  \li     Create a long sample path
  \li     Slice the long sample path from the transient time to the end
  \li     In the slice count the appearance of each state, storing in an empirical
  array
  \li     Compute the theoretical stable array
  \li     \Return Empirical stable distribution
  \li     \Return Theoretical stable distribution
\end{codebox}
\subsection*{Scripts}

\input{standardexample_scripts}
...
\begin{solution}
    ...
    \item
        ...
        \[
            P_{ij} =
            \begin{cases}
                \frac{i(N-i)}{N^2} & j = i-1 \\
                \frac{i^2}{N^2} + \frac{(N-i)^2}{N^2} & j = i \\
                \frac{i(N-i)}{N^2} & j = i+1 \\
                0 & \text{ otherwise }
            \end{cases}
        \]
...
\begin{solution}
  ...
  or more simply
  \[
    \binom{N}{i-1} \cdot \frac{N-i+1}{N} +
    \binom{N}{i+1} \cdot \frac{i+1}{N} =
    \binom{N}{i}.
  \]
  This reduces to
  \[
...
\begin{exercise}
For the third and fourth Erenfest urn models with \( N = 7 \) which
have absorbing states \( 0 \) and \( N \), find the absorption
probabilities and the expected waiting times to absorption from each
transient state.
\end{exercise}
\begin{solution}
Using the probability transition matrix from \texttt{ehrnefest3.R} and
using the \texttt{markovchain} package in R:
\begin{verbatim}
...
Using the probability transition matrix from \texttt{ehrnefest4.R} and
using the \texttt{markovchain} package in R:
\begin{verbatim}
...
\begin{exercise}
    A certain system depends on a single machine, which breaks down on
    any given day with probability \( p \).  It takes two days to
    restore the failed machine to normal service.  Form a Markov chain
    by taking as states the pairs \( (x,y) \) where \( x \) is the
    number of machines in operating condition at the end of a day and \(
    y \) is \( 1 \) if a day's labor has been expended on a machine and \(
    0 \) otherwise.  Compute the long-term system availability if \( p =
    0.01, 0.02, 0.05 \) and \( 0.10 \).
\end{exercise}
\begin{solution}
    \[
        P =
...
\begin{exercise}
    ...
    contagious disease enters the population.  During each period the
    person infects a new person with probability \( p \) or his symptoms
    appear and public health officials discover him, occurring with
    probability \( 1-p \) and quarantine him so that he is unable to
    infect additional people.  Compute the probability distribution of
    the number of infected but undiscovered people in the population at
    the time of first discovery of a carrier. Assume each infected
    person behaves like the first.
\end{exercise}
\begin{solution}
    The state space is the set of non-negative integers.  At time period
    ...
    transition probability \( p_{ik} = \binom{i}{k/2}p^{k/2}(1-p)^{i-(k/2)}
    \) for \( i \ge 0 \) and \( 0 \le k \le 2i \) and \( k \) even.
...
File name                  : standardexamples.tex
Number of characters       : 31206
Number of words            : 3657
Percent of complex words   : 18.81
Average syllables per word : 1.7555
Number of sentences        : 153
Average words per sentence : 23.9020
Number of text lines       : 656
Number of blank lines      : 129
